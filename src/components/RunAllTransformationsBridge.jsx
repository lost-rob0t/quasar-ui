...
function browserActorsSection() {
  return [...document.querySelectorAll(".graph-inspector > section")].find((section) => (
...
function summaryMessage({ actorRuns, actorsTouched, produced, failures, skipped }) {
  const base = `Ran ${actorRuns} transformation batch${actorRuns === 1 ? "" : "es"} across ${actorsTouched} actor${actorsTouched === 1 ? "" : "s"}; produced ${produced} document${produced === 1 ? "" : "s"}.`;
  const details = [];
  if (skipped) details.push(`${skipped} actor${skipped === 1 ? " was" : "s were"} already ineligible or disabled`);
  if (failures) details.push(`${failures} batch${failures === 1 ? "" : "es"} failed`);
  ...
  async function runAllTransformations() {
    ...
    try {
      for (const actor of actors || []) {
        ...
        if (customDisabled) {
          reports.push({ actorId: actor.id, status: "skipped", reason: "Custom actor execution is disabled." });
          continue;
        }
        ...
        if (!batches.length) {
          reports.push({ actorId: actor.id, status: "skipped", reason: "No eligible inputs." });
          continue;
        }

        for (const batch of batches) {
          try {
            const actorResult = await runActor(actor, batch, { quiet: true, runId: RUN_ALL_ID });
            const produced = Array.isArray(actorResult?.documents) ? actorResult.documents : [];
            corpus = mergeTransformationDocuments(corpus, produced);
      ...
      setNotice({ kind: failures.length ? "error" : "success", message });
    } catch (error) {
      setNotice({ kind: "error", message: error.message });
      setResult({ produced: 0, outputIds: [], failures: 1, message: error.message, reports });
    } finally {
      setRunning(false);
    }
  }
    ...
    <>
      ...
      >
        ...
        <span>
          ...
          <small>{inputCount} active-graph input{inputCount === 1 ? "" : "s"} · outputs feed later actors</small>
        </span>
      </button>
      {result && (
        <div className="actor-result run-all-transformations-result" role="status">
          <strong>{result.produced} document{result.produced === 1 ? "" : "s"} returned</strong>
