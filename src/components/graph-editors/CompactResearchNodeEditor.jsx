...
export default function CompactResearchNodeEditor({
  ...
  const [inputs, setInputs] = useState(formatIds(data.input_ids?.length ? data.input_ids : inputIds));
  ...
  const [maxMinutes, setMaxMinutes] = useState(Math.max(1, Math.round((limits.max_elapsed_ms ?? 1800000) / 60000)));
  ...
  const [stopWhenNoNewDocuments, setStopWhenNoNewDocuments] = useState(stop.when_no_new_documents ?? true);
  const [stopWhenSatisfied, setStopWhenSatisfied] = useState(stop.when_objective_satisfied ?? false);
  ...
  async function submit(event) {
    ...
    try {
      ...
      let next;
      if (document) {
        next = normalizeResearchNode(touchDocument(document, {
      ...
      {(requestClose) => (
        <form className="graph-compact-form" onSubmit={submit}>
          <div className="graph-editor-type-heading">
            ...
            <span>{document?.data?.status || "draft"} · {document?.dataset || dataset}</span>
          </div>

          <label className="field full">
            <span>Objective</span>
            <small>required · concise description of what this node must establish</small>
            <textarea value={objective} onChange={change(setObjective)} rows="2" autoFocus required />
          </label>
          ...
          <div className="graph-editor-fields">
            ...
            <label className="field full">
              ...
              <textarea value={actorIds} onChange={change(setActorIds)} rows="3" list="research-node-actors" />
              <datalist id="research-node-actors">
                {actors.map((actor) => <option key={actor.id} value={actor.id}>{actor.label || actor.id}</option>)}
              </datalist>
            </label>
          </div>
          ...
          <fieldset className="research-node-limits">
            ...
            <label><span>Depth</span><input type="number" min="1" value={maxDepth} onChange={change(setMaxDepth)} /></label>
            <label><span>Actor runs</span><input type="number" min="1" value={maxActorRuns} onChange={change(setMaxActorRuns)} /></label>
            <label><span>Requests</span><input type="number" min="1" value={maxRequests} onChange={change(setMaxRequests)} /></label>
            <label><span>Minutes</span><input type="number" min="1" value={maxMinutes} onChange={change(setMaxMinutes)} /></label>
            <label><span>Repeat limit</span><input type="number" min="1" value={maxRepeatedState} onChange={change(setMaxRepeatedState)} /></label>
            <label><span>Max cost</span><input type="number" min="0" step="0.01" value={maxCost} onChange={change(setMaxCost)} /></label>
          </fieldset>

          <fieldset className="research-node-stop-rules">
            <legend>Stop rules</legend>
            <label className="checkbox"><input type="checkbox" checked={stopWhenQueueEmpty} onChange={change(setStopWhenQueueEmpty)} /> Actor queue is empty</label>
            <label className="checkbox"><input type="checkbox" checked={stopWhenNoNewDocuments} onChange={change(setStopWhenNoNewDocuments)} /> No new documents</label>
            <label className="checkbox"><input type="checkbox" checked={stopWhenSatisfied} onChange={change(setStopWhenSatisfied)} /> Objective is satisfied</label>
            <label className="checkbox"><input type="checkbox" checked={haltOnFailure} onChange={change(setHaltOnFailure)} /> Actor failure</label>
          </fieldset>
