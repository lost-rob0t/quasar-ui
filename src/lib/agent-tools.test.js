...
describe("agent tools", () => {
  it("enforces permissions outside model output", () => {
    expect(() => assertToolPermission({ permissions: [] }, "documents.read")).toThrow("Permission denied");
  });
  ...
  it("queries the scoped database", async () => {
    ...
    const result = await registry.execute("query_database", {
      text: "Alice",
      datasets: ["alpha"]
    }, {
  ...
  it("runs actors only through the declared environment", async () => {
    ...
    await registry.execute("run_actor", { actorId: "actor:test" }, {
